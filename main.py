...
def camera():
    ...
    while (True): 
         ...
         #Tomamos el tamaño de la ventana del video
         fil = frame.shape[0]  
         col = frame.shape[1]
         
         #Inicializamos el limite donde pondremos nuestras lineas divisoras
         limit_right = int((col + 150) / 2)
         limit_left = int((col - 150) / 2)
    
         #Añadimos las lineas a nuestra ventana
         cv2.line(frame, (limit_right, 0), (limit_right, fil), (255, 255, 255), 2)
         cv2.line(frame, (limit_left, 0), (limit_left, fil), (255, 255, 255), 2)
         
         #Se busca el objeto del color que necesitamos y lo almacenamos en una mascara
         objectMask = cv2.inRange(frameHSV, Low, High)
         
         #Se buscan los bordes de los objetos que separamos en la mascara
         borders, _ = cv2.findContours(objectMask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
         
         #Se recorren los bordes de los objetos almacenados
         for objectBorder in borders:
            
            #Almacenamos el area del borde del objeto
            ...
            if itemSize > 300:
                ...
                #Almacenamos las coordenadas X y Y de nuestro centro respectivamente
                if (center["m00"] == 0): center["m00"] = 1
                posX = int(center["m10"] / center["m00"])
                posY = int(center["m01"] / center["m00"])
                
                #Agregamos un punto en el centro de nuestro objeto
                cv2.circle(frame, (posX, posY), 7, (0, 0, 255), -1) # 7 es el radio del circulo
                
                #Redondeamos los bordes del objeto y los dibujamos
                roundItem = cv2.convexHull(objectBorder)
                cv2.drawContours(frame, [roundItem], 0, (0, 255, 0), 5)

         #Detectamos en que posicion se encuentra el objeto y cambiamos la velocidad de la nave segun donde esta
         if (posX > 0 and posX < limit_left):
            speed_change = -5
         if (posX >= limit_left and posX <= limit_right):
            speed_change = 0
         if (posX > limit_right and posX < col):
            speed_change = +5
    ...
    #Cerramos la camara y las ventanas
    cap.release()
...
def main_menu():
    ...
    #Creamos un hilo para abrir la camara
    threadCamera = threading.Thread(target=camera)
    threadCamera.start()
